...
$cadena_script0="call reporte_ticket_control('1','$where')";
...
function Header()
{
    ...
    $this->Cell(30,10,'REPORTE CONTROL DE TICKET  - '.$nombre_clase,0,0,'C');
	...
	$this->Cell(18,6,utf8_decode('F.INGRESO'),1,0,'C',TRUE);
	$this->Cell(20,6,utf8_decode('REGISTRADO'),1,0,'C',TRUE);
	$this->Cell(20,6,utf8_decode('CLIENTE'),1,0,'C',TRUE);
	$this->Cell(60,6,utf8_decode('NOMBRE'),1,0,'C',TRUE);
	$this->Cell(42,6,utf8_decode('CLASE'),1,0,'C',TRUE);
	$this->Cell(50,6,utf8_decode('TIPO'),1,0,'C',TRUE); 
	...
	$this->Cell(22,6,utf8_decode('ESTADO'),1,1,'C',TRUE);
}
// Pie de página
function Footer()
{
...
$moneda="";
	...
	while ($row=$resultado->fetch_assoc()) {
			...
			$pdf->Cell(42,5,utf8_decode($row['clase']),0,0,'D',0);
			$pdf->Cell(50,5,substr(utf8_decode($row['tipo_ticket']),0,25),0,0,'C',0);
			// $pdf->Cell(10,5,utf8_decode($row['cantidad']),0,0,'C',0);
			// $pdf->Cell(12,5,utf8_decode($row['control']),0,0,'C',0);
			$pdf->Cell(20,5,$row['moneda'].number_format(utf8_decode($row['importe']),2),0,0,'C',0);
			// $pdf->Cell(18,5,$row['moneda'].number_format(utf8_decode($row['monto_total']),2),0,0,'C',0);
			$pdf->Cell(22,5,utf8_decode($row['estado']),0,1,'C',0);
