...
function Header()
{
	...
	$this->Cell(30,6,utf8_decode('FECHA EMISIÓN'),1,0,'C',TRUE);
	$this->Cell(15,6,utf8_decode('HORA'),1,0,'C',TRUE);
	$this->Cell(50,6,utf8_decode('TIPO TICKET'),1,0,'',TRUE);
	...
	$this->Cell(20,6,utf8_decode('N° PAGO'),1,0,'C',TRUE);
	$this->Cell(28,6,utf8_decode('CAJERO'),1,1,'C',TRUE); 
}
// Pie de página
function Footer()
{
...
$moneda='';
	...
	while ($row=$resultado->fetch_assoc()) {
			...
			$pdf->Cell(50,5,utf8_decode($row['tipo_ticket']),0,0,'',0);
			$pdf->Cell(20,5,number_format(utf8_decode($row['importe']),2),0,0,'C',0); 
			$pdf->Cell(20,5,utf8_decode($row['cantidad']),0,0,'C',0);
			$pdf->Cell(20,5,number_format(utf8_decode($row['costo_total']),2),0,0,'C',0); 
			$pdf->Cell(20,5,utf8_decode($row['forma_pago']),0,0,'C',0);
			$pdf->Cell(20,5,utf8_decode($row['referencia']),0,0,'C',0);
			$pdf->Cell(28,5,utf8_decode($row['cajero']),0,1,'C',0);
