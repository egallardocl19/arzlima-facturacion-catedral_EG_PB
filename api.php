...
if (isset($_SERVER['HTTP_ORIGIN'])){
    ...
    if ($Method == "GET")
    {
     ...
     if (!isset($_GET['tok']) && !isset($_GET['tipo_ingreso'])&& !isset($_GET['serie'])
     && !isset($_GET['submodulo'])&& !isset($_GET['fecha_inicio'])&& !isset($_GET['fecha_uso'])
     ...
     && !isset($_GET['tipopago'])&& !isset($_GET['n_pago'])&& !isset($_GET['user'])) //&&
        {
            ...
            echo json_encode($pps -> fetchAll(PDO::FETCH_OBJ));
             ...
             $consulta="CALL mantenimiento_recibo_api (:v_tok,:v_tipo_ingreso,:v_serie,:v_submodulo,
             :v_fecha_inicio,:v_fecha_uso,:v_hora_uso,:v_monto_totalxx,:v_cantidad_total,:v_dni,
            ...
            $pps-> bindParam(":v_tipo_ingreso",$_GET['tipo_ingreso']);
