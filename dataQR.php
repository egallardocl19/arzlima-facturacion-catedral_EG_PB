...
if (!empty($totalTicket)) {
	...
	//		}
	//Actualizar ticket con cantida y monto total
	//$codigo_tipo_ticket=$DataTicket['idtipo_ticket'];
	//$sql="UPDATE ticket set cantidad_total=\"$contador\",monto_total=((select importe from tipos_ticket where id=\"$codigo_tipo_ticket\")*\"$contador\") where CONCAT(serie,'-',numero)='$datoqr'";
	...
	dni,nombre,direccion,cantidad_total,idtipo_moneda,monto_total,idestado_ticket,idestado_registro,idclase_ticket,idestado_dato,observaciones,$user_id,'$created_at',2,null,2,null 
	...
	//$totalTicket2 = mysqli_num_rows($queryTicket2); //cantidad  1 o 0	
	
	//if ($DataTicket['cantidad_total'] == $totalTicket2) {
	...
	<div class="mb-5">
		<?php while ($infoTicket = mysqli_fetch_array($queryTicket)) { ?>
			...
			<?php 
			  ...
			  $messages[] = " Registrado Exitosamente para el Ingreso.".'<br>'.'<br>';
			  $consulta_detalle="SELECT concat(t.serie,'-',t.numero) as ticket, tt.nombre, td.cantidad FROM ticket_detalle td, ticket t, tipos_ticket tt 
			  where t.id=td.id_ticket and td.idtipo_ticket=tt.id and concat(t.serie,'-',t.numero)='$datoqr'";
			  $resultado_detalle=$con->query($consulta_detalle);
			  while ($row=$resultado_detalle->fetch_assoc()) {
			  $messages[] = "# ".$row['nombre'].' - Cantidad: '.$row['cantidad'].'<br>';
			  }
			?>
...
<?php 
				...
				?>
				<div class="alert alert-danger" role="alert" style="font-size: 20;">
				...
				if (isset($messages)){
					...
					?>
					<div class="alert alert-success" role="alert" style="font-size: 20;">
							<button type="button" class="close" data-dismiss="alert">&times;</button>
