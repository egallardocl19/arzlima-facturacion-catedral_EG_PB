<?php	
	...
	if (empty($_POST['anio_rat'])){
		...
		$dia_condicion = date("d", strtotime($fecha_inicio)); 
		$mes_condicion = date("m", strtotime($fecha_inicio));
		$anio_condicion = date("Y", strtotime($fecha_inicio)); 

		$dia_condicion2 = date("d", strtotime($fecha_vencimiento)); 
		$mes_condicion2 = date("m", strtotime($fecha_vencimiento));
		$anio_condicion2 = date("Y", strtotime($fecha_vencimiento));   
			...
			if (($anio-$anio_actual)==0){
				if (($mes-$mes_actual)==0){
					...
					if (!$sql2|| mysqli_num_rows($sql2)!=0){
						...
						codigo_recibo=r.codigo_recibo)) as glosa,cc.id as idcontabilidad_concar,r.tipo_recibo,r.codigo_recibo,'$iduser_add','$fecha_add' 
						...
						cc.tipo_cuenta='D' and r.tipo_recibo='RAT' date_format(r.fecha_recibo, '%m')='$mes' and date_format(r.fecha_recibo, '%Y')='$anio' order  by r.codigo_recibo)";
